...
(async function () {
    ...
    let boxObstacle: unknown = nav.addBoxObstacle(new Vector3(-2, 1, 1), new Vector3(1, 1, 1), 0.2);
    ...
    type WireMesh = ReturnType<typeof MeshBuilder.CreateTube>;

    function addEdgeTube(edgeStart: Vector3, edgeEnd: Vector3, position: Vector3, rotationY: number): WireMesh {
    ...
    function addBoxWireframe(half: Vector3, position: Vector3, rotationY: number): WireMesh[] {
        const corners = [
    ...
    let boxTubes: WireMesh[] | null = addBoxWireframe(new Vector3(1, 1, 1), new Vector3(-2, 1, 1), 0.2);

    let navDebug = nav.createDebugNavMesh(scene);
    ...
    if (freeze) {
        ...
        setTimeout(() => {
            if (boxObstacle) {
                nav.removeObstacle(boxObstacle as never);
                boxObstacle = null;
                WaitForFullTileCacheUpdate(navResult.navMesh, navResult.tileCache);
                navDebug.dispose();
                navDebug = nav.createDebugNavMesh(scene);
                navDebug.position.y = 0.02;
                navDebug.material = navDebugMat;
            }
            if (boxTubes) {
                for (const t of boxTubes) t.dispose();
                boxTubes = null;
            }
        }, 1000);
    }
